...
public class SMSServer {
        ...
        try {
            ...
            if (System.getProperty("smsserver.configdir") != null)
                f = new FileInputStream(
                    System.getProperty("smsserver.configdir") + "SMSServer.conf");
                ...
                try {
                    Class<?> c = Class.forName(
                        (getProperties().getProperty(
                            ...
                            .indexOf('.') == -1 ? "org.smslib.balancing." : "") + getProperties()
                            .getProperty(
                                "smsserver.balancer", ""
                            )
                    );
                    Constructor<?> constructor = c.getConstructor();
                    org.smslib.balancing.LoadBalancer balancer = (org.smslib.balancing
                        .LoadBalancer) constructor
                        .newInstance();
                    Service.getInstance().setLoadBalancer(balancer);
                    Logger.getInstance()
                        .logInfo(
                            "SMSServer: set balancer to: " + getProperties().getProperty(
                                "smsserver.balancer",
                                ""
                            ), null, null
                        );
                    ...
                    e.printStackTrace();
                    Logger.getInstance()
                        .logError("SMSServer: error setting custom balancer!", null, null);
                }
            }
            if (getProperties().getProperty("smsserver.router", "").length() > 0) {
                try {
                    Class<?> c = Class.forName(
                        (getProperties().getProperty("smsserver.router", "")
                            .indexOf('.') == -1 ? "org.smslib.routing." : "") + getProperties()
                            .getProperty(
                                "smsserver.router", ""
                            )
                    );
                    ...
                    Logger.getInstance()
                        .logInfo(
                            "SMSServer: set router to: " + getProperties().getProperty(
                                "smsserver.router",
                                ""
                            ), null, null
                        );
            ...
            for (int i = 0; i < Integer.MAX_VALUE; i++) {
                try {
                    ...
                    Class<?> c = Class.forName((gtwClass.indexOf('.') == -1 ? "org.smslib" +
                        ".smsserver.gateways." : "") + gtwClass);
                    ...
                    Logger.getInstance()
                        .logInfo(
                            "SMSServer: added gateway " + gtwId + " / " + gtw.getDescription(),
                            null,
                            null
                        );
                } catch (Exception e) {
                    Logger.getInstance()
                        .logError("SMSServer: Unknown Gateway in configuration file!", null, null);
                }
            }
            for (int i = 0; i < Integer.MAX_VALUE; i++) {
                try {
                    ...
                    Class<?> c = Class.forName((infClass.indexOf('.') == -1 ? "org.smslib" +
                        ".smsserver.interfaces." : "") + infClass);
                    Constructor<?> constructor = c.getConstructor(argsClass);
                    ...
                    Logger.getInstance()
                        .logInfo(
                            "SMSServer: added interface " + infId + " / " + inf.getDescription()
                                + " / " + inf
                                .getType(), null, null
                        );
                }
                /* Check for exceptions thrown by the constructor itself */ catch
                    (InvocationTargetException e) {
                    Logger.getInstance()
                        .logError(
                            "SMSServer: Illegal Interface configuration: " + e.getCause()
    ...
    private void process() throws Exception {
        ...
        while (!this.shutdown) Thread.sleep(1000);
    }
    ...
    void readMessages() {
        ...
        try {
            ...
            if (msgList.size() > 0) {
                ...
                if (getProperties().getProperty("settings.delete_after_processing", "no")
            ...
            if (getProperties().getProperty("settings.send_mode", "sync")
                ...
                for (OutboundMessage msg : msgList) {
                    try {
                        ...
                        Logger.getInstance()
                            .logError("SMSServer: sending messages exception!", e, null);
                        try {
                            for (Interface<? extends Object> inf : getInfList())
                                if (inf.isOutbound()) inf.markMessage(msg);
                        } catch (Exception e1) {
                            Logger.getInstance()
                                .logError("SMSServer: sending messages exception!", e1, null);
                        }
                    }
                }
            } else {
                if (msgList.size() != 0) {
                    Logger.getInstance()
                        .logInfo(
                            "SMSServer: sending asynchronously... [" + msgList.size() + "]",
                            null,
                            null
                        );
                }
    ...
    public boolean checkPriorityTimeFrame(int priority) {
        ...
        current = cal.get(Calendar.HOUR_OF_DAY) < 10 ? "0" + cal.get(Calendar.HOUR_OF_DAY) : "" + cal
            .get(Calendar.HOUR_OF_DAY);
        current += cal.get(Calendar.MINUTE) < 10 ? "0" + cal.get(Calendar.MINUTE) : "" + cal.get(
            Calendar.MINUTE
        );
        if ((Integer.parseInt(current) >= Integer.parseInt(from)) && (Integer.parseInt(current) < Integer
            .parseInt(to))) return true;
        return false;
    }
