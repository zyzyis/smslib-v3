                ...
                Logger.getInstance()
                    .logError(
                        String.format(
                            ...
                            (sqlDelayMultiplier * (SQL_DELAY / 1000))
                        ), e, null
                    );
            ...
            try {
                ...
                pst = con.prepareStatement(
                    " insert into " + getProperty(
                        ...
                        "" + " original_ref_no, original_receive_date, " +
                        "gateway_id) " + " values(?,?,?,?,?,?,?,?,?,?)"
                );
                for (InboundMessage msg : msgList) {
                    if ((msg.getType() == MessageTypes.INBOUND) || (msg.getType() == MessageTypes
                        .STATUSREPORT)) {
                        pst.setInt(1, 0);
                        ...
                        switch (msg.getType()) {
                            ...
                            case STATUSREPORT:
                                ...
                                if (((StatusReportMessage) msg).getSent() != null) pst.setTimestamp(
                                    5,
                                    new Timestamp(((StatusReportMessage) msg).getSent().getTime())
                                );
                                ...
                                if (((StatusReportMessage) msg).getReceived() != null)
                                    pst.setTimestamp(
                                        9,
                                        new Timestamp(
                                            ((StatusReportMessage) msg).getReceived()
                                                .getTime()
                                        )
                                    );
                                if (getProperty(
                                    "update_outbound_on_statusreport",
                                    "no"
                                ).equalsIgnoreCase("yes")) {
                                    PreparedStatement cmd2;
                                    cmd2 = con.prepareStatement(
                                        " update " + getProperty(
                                            "tables.sms_out",
                                            "smsserver_out"
                                        ) + " set status = ? " + " where (recipient = ? or " +
                                            "recipient = ?) and ref_no = ? and gateway_id = ?"
                                    );
                                    ...
                                    if (((StatusReportMessage) msg).getRecipient().startsWith("+"))
                                        cmd2.setString(
                                            3,
                                            ((StatusReportMessage) msg).getRecipient().substring(1)
                                        );
                                    else cmd2.setString(
                                        3,
    ...
    public Collection<OutboundMessage> getMessagesToSend() throws Exception {
        ...
        while (true) {
            try {
                ...
                rs = cmd.executeQuery(
                    "select id, type, recipient, text, wap_url, wap_expiry_date, wap_signal, " +
                        "create_date, originator, encoding, status_report, flash_sms, src_port, " +
                        "dst_port, sent_date, ref_no, priority, status, errors, " +
                        "gateway_id from " + getProperty(
                        "tables.sms_out",
                        "smsserver_out"
                    ...
                    if (getServer().checkPriorityTimeFrame(rs.getInt("priority"))) {
                        switch (rs.getString("type").charAt(0)) {
                            ...
                            case 'W':
                                ...
                                msg = new OutboundWapSIMessage(
                                    rs.getString("recipient").trim(),
                                    new URL(
                                        rs.getString("wap_url")
                                            .trim()
                                    ),
                                    rs.getString("text").trim(),
                                    wapExpiryDate,
                        ...
                        if ((rs.getString("originator") != null) && (rs.getString("originator")
                            .length() > 0)) msg.setFrom(rs.getString("originator").trim());
    ...
    public int getPendingMessagesToSend() throws Exception {
        ...
        while (true) {
            try {
                ...
                rs = cmd.executeQuery(
                    "select count(*) as cnt from " + getProperty(
                        "tables.sms_out",
                        "smsserver_out"
                    ) + " where status in ('U', 'Q')"
                );
        ...
        while (true) {
            try {
                ...
                selectStatement = con.prepareStatement(
                    "select errors from " + getProperty(
                        "tables.sms_out",
                        "smsserver_out"
                    ) + " where id = ?"
                );
                ...
                } else if ((msg.getMessageStatus() == MessageStatuses.UNSENT) || ((msg.getMessageStatus() == MessageStatuses.FAILED) && (msg
